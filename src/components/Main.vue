...
<template>
  ...
  <Modal ref="modalRef">
    <div class="modal_inners">
      <VueDraggableNext tag="ul" class="weather-card-settings_list" :list="weatherRef" @update="handleDrag">
        <TransitionGroup name="slide">
          <div class="city_settings" draggable="true" v-for="item in weatherRef" :key="item.name">{{ item.name }}, {{ item.sys.country }}</div>
        </TransitionGroup>
      </VueDraggableNext>
      <form @submit.prevent="() => getWeatherOnSubmit(townToAdd)">
        <Input v-model="townToAdd" name="Add city weather" placeholder="Enter really existing city" :disabled="isLoading" />
      </form>
    </div>
  </Modal>
</template>
